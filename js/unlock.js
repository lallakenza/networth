...
// './data.js' et './data.js?v=524' sont DEUX modules distincts, donc deux objets PORTFOLIO
...
import * as DATA from './data.js?v=524';
...
export async function blobDisponible() {
  ...
  //
  // try { DATA_ENC = (await import('./data.enc.js?v=524')).DATA_ENC; return !!DATA_ENC; }
...
/**
 * Déverrouille en récupérant la clé auprès de Supabase, après authentification.
 * C'est le chemin principal depuis v511 : la clé est un secret aléatoire, pas une phrase
 * humaine. Elle est ensuite mémorisée sur l'appareil comme l'était la phrase, ce qui rend le
 * code court à nouveau suffisant au quotidien.
 */
export async function deverrouillerDepuisServeur() {
