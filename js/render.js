...
import { CURRENCY_CONFIG, CASH_YIELDS, IMMO_CONSTANTS, EXIT_COSTS, VITRY_CONSTRAINTS, IMMO_PRESETS, FX_STATIC, DECLARED_MONTHLY_SAVINGS_EUR, DESIGN_TOKENS, MARGIN_RATES, IMMO_PASSIFS_DOCUMENTES, INFLATION_RATE, VILLEJUIF_CONSTRAINTS, VILLEJUIF_ACTE, RESIDENCE_FISCALE } from './data.js?v=546';
import { getGrandTotal, computeImmoFinancing, computeCashFlow, computeAlerts, computeObjectifs, computeSensibilite, computeFiscaliteMRE, computeExitCostsAtYear, computeScenarioTauxImmo, projectNW, sadevFenetre } from './engine.js?v=546';
...
export function render(state, view, currency) {
  ...
  if (nwHistCanvas) {
    ...
    const snaps = (window._nwSnapCache || []).filter(r => r.data && ((r.data.total && r.data.total.couple != null)
      || (r.data.totalReconstitue && r.data.totalReconstitue.amine != null)));   // v448 — reconstitué compte aussi
...
function renderExpandSubs(state, view, options = {}) {
  ...
  if (ibkrBadge) {
    const chartTWR = window._chartKPIData?.twr;
    const twrBadge = chartTWR != null ? chartTWR : (p0.amine.ibkr.meta.twr || 0);
    ibkrBadge.textContent = 'TWR ' + (twrBadge >= 0 ? '+' : '') + twrBadge.toFixed(2) + '%';
  }
...
export function applySnapshotDeltas(s, vue) {
  try {
    ...
    // v391 — sauter les snapshots rétroactifs (backfill positions : pas de total NW)
    const base = [...cache].reverse().find(r => r.date < todayParis && r.data && r.data.total && r.data.total.couple != null);
    if (!base) return;
...
function renderActionsView(state) {
  ...
  const _twrDisp = window._chartKPIData?.twr ?? av.twr;
  setText('kpiActionsTWR', 'TWR ' + (_twrDisp >= 0 ? '+' : '') + _twrDisp.toFixed(1) + '%');
  ...
  if (av.periodPL) {
    ...
    // v405 (BUG-079) — 'kpiPLDaily' RETIRÉ : le daily vient désormais de l'engine
    // (previousClose live vs prix live), seule source fiable à l'échelle d'une séance et
    // identique au panneau de détail. Le graphe reste maître des périodes plus longues.
    const chartOverriddenKPIs = new Set(['kpiPLMTD', 'kpiPL1M', 'kpiPLYTD', 'kpiPL1Y']);
    [
...
function setupKPIDetailPanels(state) {
  ...
  // Cost items are now computed by engine.js and included directly in d.breakdown
  // No need for appendCostItems — the engine adds FTT, commissions, interest, dividends
  ...
  const detailGenerators = {
    ...
    detailPLMTD: function() {
      const cb = window._chartBreakdown?.mtd;
      if (cb?.hasData) {
        const items = cb.breakdown;
        const losers = items.filter(i => i.pl < 0);
        const worst3 = losers.slice(0, 3).map(i => i.label + ' (' + fmt(Math.round(i.pl)) + ')').join(', ');
    ...
    detailPL1M: function() {
      const cb = window._chartBreakdown?.oneMonth;
      if (cb?.hasData) {
        const items = cb.breakdown;
        const losers = items.filter(i => i.pl < 0);
        const worst3 = losers.slice(0, 3).map(i => i.label + ' (' + fmt(Math.round(i.pl)) + ')').join(', ');
    ...
    detailDividends: function() {
      const chartTWR = window._chartKPIData?.twr;
      const twrVal = chartTWR != null ? chartTWR : av.twr;
      let html = '<div class="detail-header"><h4>Dividendes & Performance</h4>';
      html += '<div class="detail-summary">TWR ' + (twrVal >= 0 ? '+' : '') + twrVal.toFixed(1) + '% | Dividendes nets ' + fmt(Math.round(av.dividends)) + '</div></div>';
      html += '<div class="detail-body">';
      html += '<div class="detail-row"><span class="ticker-label">Dividendes nets reçus (all-time)</span><span class="ticker-pl pl-pos">+' + fmt(Math.round(av.dividends)) + '</span><span class="ticker-bar"></span></div>';
      html += '<div class="detail-row"><span class="ticker-label">Commissions payées (all-time)</span><span class="ticker-pl pl-neg">' + fmt(Math.round(av.commissions || 0)) + '</span><span class="ticker-bar"></span></div>';
      html += '<div class="detail-row"><span class="ticker-label">FTT payé (all-time)</span><span class="ticker-pl pl-neg">' + fmt(Math.round(av.fttAllTime || 0)) + '</span><span class="ticker-bar"></span></div>';
      html += '<div class="detail-row"><span class="ticker-label">Intérêts marge (all-time)</span><span class="ticker-pl pl-neg">' + fmt(Math.round(av.interestAllTime || 0)) + '</span><span class="ticker-bar"></span></div>';
      ...
      html += '📊 <b>TWR (Time-Weighted Return)</b> mesure la performance pure du portefeuille en neutralisant l\'effet des dépôts/retraits. Calculé par chainage quotidien : ∏(NAV[j] / (NAV[j-1] + dépôt[j])) - 1. C\'est le standard GIPS utilisé par IBKR.';
      html += '<br>💡 WHT (retenue à la source) déduite automatiquement sur dividendes FR (30%), US (15%), JP (15%). En tant que résident fiscal UAE, vendre avant l\'ex-date évite la WHT.';
      html += '</div>';
      return html;
    },
  };
...
function attachKPIInsights(state, view) {
  ...
  if (s.immoView) {
    ...
    insights['kpiImmoViewVal'] = 'Valeur march\u00e9 bas\u00e9e sur donn\u00e9es MeilleursAgents/efficity (mars 2026) + prime neuf (+12-15%). Vitry 1.5%/an (L15 Les Ardoines), Villejuif 1.7%/an (L15 Louis Aragon), Rueil 1%/an (L15 lointaine).';
...
function renderImmoFinancingView(state) {
  ...
  // Sans ça, tous les boutons partent neutres et l'utilisateur ne sait pas
  // lequel est actif tant qu'il n'a pas cliqué.
