...
## BUG-090: Graphes immo « disparus » — canvas 0×0 dans les accordéons repliés

- **Version**: détecté v471, corrigé v472 (26 août 2026). Introduit par la réorg accordéons v429.
- **Sévérité**: MOYENNE — la projection Création de richesse (20 ans), la projection équité et les graphes CF étaient invisibles (canvas 0×0) même après ouverture de l'accordéon.
- **Détection**: Amine — « je veux restaurer la courbe de création de richesse dans l'immo. Je la trouve plus dans cette page ».
- **Symptôme**: ouvrir « Création de richesse » (#secImmoRichesse) affichait les sections mais les 2 canvas restaient à 0×0 (mesuré live).
...
## BUG-092: Trois formules « plus-value brute » divergentes — la même vente Vitry affichait +7 641 (fiche) et −7 522 (plan fiscal)

- **Version**: détecté v473 (question d'Amine sur PV +7 641 vs valeur 303 266), corrigé v474 (26 août 2026).
- **Sévérité**: MOYENNE-HAUTE — contradiction inter-pages sur un montant fiscal ; impôt PV affiché 2 766 € sur une page, 0 € sur l'autre.
- **Root cause**: trois implémentations indépendantes de la PV immobilière : (1) `computeExitCosts` (fiche, assiette acq. +7,5 %, vente directe) ; (2) formule locale de `computeFiscaliteMRE` (déduisait EN PLUS 5 % de frais d'agence hypothétiques + fallbacks hardcodés `280000`/`300000`/`purchaseDate '2019-12-15'` — la date de Rueil pour un calcul Vitry) ; (3) copie locale LMNP dans render (barème d'abattements off-by-one, surtaxe absente). Le libellé « Plus-value brute » à côté de « Valeur actuelle » laissait aussi croire à une contradiction alors que +7 641 est l'assiette fiscale (économique : +28 266).
- **Fix (v474)**: source unique. `computeFiscaliteMRE.pvVitry` lit désormais `vitry.exitCosts` (mêmes assiette/barème/doctrine que la fiche) ; la vente via agence devient une VARIANTE étiquetée (frais de cession déduits de l'assiette, art. 150 VA, taux `EXIT_COSTS.agencyFeePct`) ; fallbacks hardcodés supprimés (null si exitCosts absent). Table LMNP Rueil : PV + taxe via `computeExitCostsAtYear('rueil', …, crd=0)`. Fiche : deux lectures étiquetées « PV économique » (+28 266) et « PV imposable brute (acq. +7,5 %) » (+7 641) avec tooltips d'assiette.
- **Regression tests**: fiche Vitry → PV économique ≈ valeur − 275 000 ET PV imposable ≈ valeur − 295 625, cohérentes ; plan fiscal « PV Vitry si vente » = mêmes pvBrute/impôt que la fiche + ligne variante agence ; table LMNP Rueil rend des taxes issues du barème EXIT_COSTS (surtaxe incluse >50K).

## BUG-093: Tableau consolidé couple — Banque Populaire + Binance USDT absents (Σ lignes ≠ NW de 5 209 €)

- **Version**: détecté v479 (audit BI dual-agent de la page d'accueil), corrigé v480 (28 août 2026).
- **Sévérité**: HAUTE — le hero et le tableau « Détail consolidé » affichaient deux Net Worth différents (744 239 vs 739 082 mesurés live) ; le garde-fou BUG-064 le signalait en console mais rien à l'écran.
...
## BUG-103 à BUG-110 : lot des trouvailles moyennes et basses de l'audit du 29/08

Corrigés en v494. Chaque point ci-dessous a été vérifié par exécution du moteur réel.

- **BUG-103 (data.js)** — Commission d'un change libellée en yens mais saisie en euros. Le trade EUR.JPY du 08/04/2026 portait `commission: -319.17` alors que toutes les autres conversions coûtent ~1,72 € ; le connecteur IBKR facture bien ces opérations en JPY (318,8 JPY sur le trade comparable du 28/08). **Effet** : 319 € de coût fictif, soit 58 % du total des commissions affiché. **Fix** : −1,74 €. Commissions all-time −544,89 → **−227,46 €**. *Note honnête : l'invariant comptable passe de Δ −111 à Δ −428 € (tolérance ±5 000) — la correction est fondée sur une preuve directe, mais l'écart révèle un poste non tracé ailleurs, à investiguer.*
- **BUG-104 (data.js)** — Assurance emprunteur de Villejuif : 53,57 €/mois (montant contractuel de l'offre) dans `IMMO_CONSTANTS.charges` contre 51,29 (relevés + échéancier) dans `villejuifInsurance`. Le CF projeté s'appuyait sur l'un, le CF réel sur l'autre. **Fix** : le constaté (51,29) partout.
- **BUG-105 (engine.js)** — Retenue à la source déduite de la PLACE DE COTATION et non du pays de source. Airbus, cotée à Paris mais néerlandaise, était projetée à 25 % alors que le versement du 23/04/2026 enregistre 15 % (« €640 brut − €96 WHT 15% NL »). **Fix** : champ `whtCountry` sur la position, taux `netherlands: 0.15` ajouté, `WHT_RATES[pos.whtCountry || pos.geo]` aux deux points de calcul.
- **BUG-106 (engine.js)** — `recvOmarMAD` : un `|| 28000` codé en dur se déclenchait sur un total légitimement NUL. Passer la créance Omar en « recouvré » — le geste de maintenance normal — faisait réapparaître 28 000 MAD dans le tableau consolidé. **Fix** : plus de repli.
- **BUG-107 (engine.js)** — Les deux camemberts de la page Actions ne totalisaient pas la même chose : l'ESPP de Nezha (5 140 €) manquait au sectoriel, et le SGTM (4 205 €) n'avait aucun secteur. **Fix** : ESPP ajouté, SGTM classé en `industrials` (BTP) dans les deux implémentations. Vérifié : Σ géo = Σ secteur = 253 442 €, et la partition amine+nezha égale exactement le total couple.
- **BUG-108 (render.js)** — Trois valeurs du même « manque à gagner » sur la trésorerie : 0,05 codé en dur dans l'insight, `cv.refYield` (0,06) dans la table. 5 120 € annoncés contre 5 948 €. **Fix** : `cv.refYield` partout.
- **BUG-109 (render.js)** — Tableau de projection de Nezha : la ligne « Equity Villejuif » affichait 0 sur toutes les colonnes antérieures à 2028, alors que le capital engagé (44,5 K€) est déjà au patrimoine. Le total projeté démarrait **sous** le patrimoine actuel. **Fix** : engagé jusqu'à la livraison puis croissance — même doctrine que le simulateur (v490) et `projectNW`.
- **BUG-110 (render.js)** — Les réglages du graphe « Création de richesse » (Par an/Par mois, Par type/Par appart) étaient des variables locales à `renderImmoView`, rejoué à chaque rafraîchissement : le choix de l'utilisateur se réinitialisait seul toutes les 5 minutes. **Fix** : état mémorisé au niveau module + synchronisation visuelle des boutons au re-rendu.
